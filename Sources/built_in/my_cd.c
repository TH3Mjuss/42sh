...
** Last update Wed Apr 23 12:59:24 2014 Zackary Beaugelin
*/

#include "my_sh.h"

char	*my_find(char **env, int k, char *to_find, t_bfree *bf)
{
  char	**directory;
  ...
  while (env[k])
      ...
      if (my_strncmp(to_find, env[k], my_strlen(to_find)) == 0)
	{
	  ...
	  bf->dir_ret = xmalloc(my_strlen(directory[0]));
	  bf->dir_ret = directory[0];
	  return (bf->dir_ret);
	}
      k++;
    }
...
char	**my_cd(char **param, char **env, t_bfree *bf)
{
  char  *dir;
