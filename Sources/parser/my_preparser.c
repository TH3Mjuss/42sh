...
** Last update Sun May 25 22:34:57 2014 Zackary Beaugelin
*/
...
int	my_preparser2(char **cmd, char **tok, char **env, t_parse *p)
{
  ...
  if (!my_strcmp(tok[p->i], "<"))
    ...
    {
      if ((ret = my_parser(my_str_to_wordtab(cmd[p->j], ' ', 0, 0), 0, env) != -1))
	ret = my_parser(my_str_to_wordtab(cmd[p->j + 1], ' ', 0, 0), 0, env);
    }
  else if (!tok[p->i])
